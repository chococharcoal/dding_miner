...
export const FARMING = {
  STAMINA_PER_USE: 7,        // 채집 1회 스태미나 소모
  ...
  /* 대왕 작물 */
  KING_CROP_BASE_PCT: 0.02, // 기본 0.02% 확률
  KING_CROP_MULT:     7,    // 일반 수확량의 7배 드롭
};
...
export const ENGRAVING = {
  ...
  /* 🌱 작물 상자 — 씨앗 1~50개 기댓값 25.5 */
  CROP_BOX: { pct: [0, 1, 2, 3, 4, 5], avgSeeds: 25.5 },

  /* 🎲 농부 룰렛 — 주사위 눈×6 씨앗, 골드주사위 10% → 눈×12 */
  FARMER_ROULETTE: {
    dicePct:    [0, 1, 2, 3, 4, 5],
    normalMult: 6,
    goldenMult: 12,
...
/* ════════════════════════════════════════
   요리 레시피
   ★ currentPrice — 가격 변동 시 수동 업데이트 (원/개)
   ★ base 원가 — 씨앗 가격 기반으로 자동 계산 (씨앗가 미입력 시 0원)
   ★ crops 수량 = 묶음(세트) 기준
   ★ other 수량 = 개 기준 (가격은 세트당 입력, 내부에서 ÷64 처리)
════════════════════════════════════════ */
...
/* ════════════════════════════════════════
   기본 시세 — 자동채우기 기준값
   ★ seeds  : 원/개  → 자동채우기 시 ×64 하여 세트당 필드에 채움
   ★ crops  : 원/개  → 자동채우기 시 ×64 하여 세트당 필드에 채움
   ★ other  : 원/세트 → 자동채우기 시 그대로 세트당 필드에 채움
   ★ bases  : 씨앗 가격으로 자동 계산 (항목 없음)
   ★ 3일에 1번 수정 예정
════════════════════════════════════════ */
export const DEFAULT_PRICES = {
  seeds: { tomato: 0, onion: 0, garlic: 0 },  // 원/개 → ×64 = 세트당
  crops: {                                      // 원/개 → ×64 = 세트당
    pumpkin: 0, potato: 0, carrot: 0,
    beet: 0, watermelon: 0, sweetfruit: 0,
  },
  other: {                                      // 원/세트 (세트당 직접 입력)
