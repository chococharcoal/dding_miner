...
export function co() {
  ...
  function buildPlanItems(results) {
    ...
    return results.map(c => {
      ...
      const apprHtml = c.type === 'precious'
        ? `<div class="plan-appr-row"><span>하 ${f(c.extra.item.prices.LOW*(1+pb))}원</span><span>우수 ${f(c.extra.item.prices.GOOD*(1+pb))}원</span><span>황실 ${f(c.extra.item.prices.ROYAL*(1+pb))}원</span><span class="best">기댓값 ${f(c.extra.avgSell)}원</span></div>` : '';
      const timeHtml = t > 0 ? `<div class="plan-item-time">⏱ ${fmtTime(t)}</div>` : '';
      return `<div class="plan-craft-item">
        <div class="plan-item-head">
          <div class="plan-item-name">${c.label}${precBadge}${npiTxt ? `<span class="npi">${npiTxt}</span>` : ''}</div>
  ...
  function matSummaryHtml(results, remC, remR, remS) {
    ...
    return `<div class="mat-toggle" onclick="this.nextElementSibling.classList.toggle('open');this.querySelector('.mat-arr').textContent=this.nextElementSibling.classList.contains('open')?'▲':'▼'">
  ...
  function planSummaryHtml(rev, time, remC, remR, remS) {
    const remRow = (remC + remR + remS) > 0 ? `<div class="plan-summary-row"><span class="sl">남은 주괴</span><span class="sv" style="font-size:11px">${remC > 0 ? `<span style="color:${CC}">${f(remC)}개</span> ` : ''}${remR > 0 ? `<span style="color:${CR}">${f(remR)}개</span> ` : ''}${remS > 0 ? `<span style="color:${CS}">${f(remS)}개</span>` : ''}</span></div>` : '';
    const timeRow = time > 0 ? `<div class="plan-summary-row"><span class="sl">총 제작 시간</span><span class="sv">${fmtTime(time)}</span></div>` : '';
    return `<div class="plan-summary">${remRow}${timeRow}<div class="plan-summary-row rev-row"><span class="sl">예상 수익</span><span class="sv">${f(rev)}원</span></div></div>`;
  }
  ...
  const netSummaryRow = (label, net, iC, iR, iS) => {
    ...
    const ingotKindLabel = ingotKindCount === 1 ? (iC > 0 ? '코룸 ' : iR > 0 ? '리프톤 ' : '세렌트 ') : '';
    const perHtml = n > 0 ? ` <small style="color:var(--muted)">· ${ingotKindLabel}주괴당 ${f(net / n)}원</small>` : '';
    return row(`${label}`, `${f(net)}원 ${perHtml}`, color);
  };
  const noSwapInner = `<div class="plan-items-wrap">${craftLines}${matSummaryHtml(craftResult, remCo, remRi, remSe)}</div>${planSummaryHtml(craftRev, craftTime, remCo, remRi, remSe)}`;
  const swapInner = (() => {
    if (!swapResult) return `<div class="plan-items-wrap"><div style="padding:20px 0;text-align:center;color:var(--muted);font-size:12px">교환으로 이득을 볼 수 있는<br>주괴 조합이 없습니다</div></div>`;
    const kindColor = { '코룸': CC, '리프톤': CR, '세렌트': CS };
    const swapInfoRows = swapResult.swapLog.map(s =>
      `<div class="swap-info-row"><span class="si-label"><span style="color:${kindColor[s.from]}">${s.from}</span><span style="color:var(--muted)"> → </span><span style="color:${kindColor[s.to]}">${s.to}</span>&nbsp;<b>${f(s.count)}개</b> 교환</span></div>`
    ).join('');
    return `<div class="plan-items-wrap"><div class="swap-info-box"><div class="swap-info-box-title">교환</div>${swapInfoRows}</div>${swapCraftLines}${matSummaryHtml(swapCraftResult, swapRemCo, swapRemRi, swapRemSe)}</div>${planSummaryHtml(swapCraftRev, swapCraftTime, swapRemCo, swapRemRi, swapRemSe)}`;
  })();
  const noSwapBadge = (noSwapWinner && swapResult) ? ` ${bdg('bg', `+${f(craftRev - swapCraftRev)}원`)}` : '';
  const swapBadge   = swapBetter ? ` ${bdg('bg', `+${f(swapCraftRev - craftRev)}원`)}` : '';
  document.getElementById('oRes').innerHTML = `
  <div class="rsec">
    <div class="rsec-title">📦 보유 주괴 &amp; 단가</div>
    ${iCo > 0 ? `<div class="rrow"><span class="rl" style="color:${CC};font-weight:700">코룸</span><span class="rv">${f(iCo)}개 <span class="muted" style="font-size:11px;font-weight:500">(개당 ${f(cP)}원)</span></span></div>` : ''}
    ${iRi > 0 ? `<div class="rrow"><span class="rl" style="color:${CR};font-weight:700">리프톤</span><span class="rv">${f(iRi)}개 <span class="muted" style="font-size:11px;font-weight:500">(개당 ${f(rP)}원)</span></span></div>` : ''}
    ${iSe > 0 ? `<div class="rrow"><span class="rl" style="color:${CS};font-weight:700">세렌트</span><span class="rv">${f(iSe)}개 <span class="muted" style="font-size:11px;font-weight:500">(개당 ${f(sP)}원)</span></span></div>` : ''}
  ...
  <div class="rsec" style="padding:0;overflow:hidden">
    <div style="padding:10px 12px 8px;border-bottom:1px solid var(--bdr2);display:flex;align-items:center;gap:8px;flex-wrap:wrap">
      <span style="font-size:15px;color:var(--txt)">🔨 최적 제작 계획</span>${fBdg}
    </div>
    <div class="plan-cols">
      <div class="plan-col${noSwapWinner ? ' winner' : ''}">
        <div class="plan-col-title${noSwapWinner ? ' winner-title' : ' loser-title'}">${noSwapWinner ? '✓ ' : ''}교환 X${noSwapBadge}</div>
        ${noSwapInner}
      </div>
      <div class="plan-col${swapBetter ? ' winner' : ''}">
        <div class="plan-col-title${swapBetter ? ' winner-title' : ' loser-title'}">${swapBetter ? '✓ ' : ''}교환 O${swapBadge}</div>
        ${swapInner}
      </div>
    </div>
  </div>
